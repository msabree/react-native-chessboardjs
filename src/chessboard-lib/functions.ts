...
export function getRelativeCoords(
  ...
  const col = square[0];
  const row = square[1];
  if (col === undefined || row === undefined) return { x: 0, y: 0 };
  const colVal = columns[col];
  const rowVal = rows[parseInt(row, 10) - 1];
  if (colVal === undefined || rowVal === undefined) return { x: 0, y: 0 };
  const x = colVal * squareWidth + squareWidth / 2;
  const y = rowVal * squareWidth + squareWidth / 2;
  return { x, y };
}
...
export function isDifferentFromStart(newPosition: BoardPosition): boolean {
  ...
  (
    Object.keys(START_POSITION_OBJECT) as (keyof typeof START_POSITION_OBJECT)[]
  ).forEach((square) => {
    if (newPosition[square] !== START_POSITION_OBJECT[square])
      isDifferent = true;
  });

  (Object.keys(newPosition) as (keyof typeof newPosition)[]).forEach(
    (square) => {
...
export function getPositionDifferences(
  ...
  (Object.keys(currentPosition) as (keyof typeof currentPosition)[]).forEach(
    (square) => {
...
function fenToObj(fen: string): BoardPosition {
  ...
  for (let i = 0; i < 8; i++) {
    const rowChars = rows[i];
    if (!rowChars) continue;
    const row = rowChars.split("");
    let colIdx = 0;
    ...
    for (let j = 0; j < row.length; j++) {
      ...
      if (char.search(/[1-8]/) !== -1) {
        const numEmptySquares = parseInt(char, 10);
        colIdx = colIdx + numEmptySquares;
      } else {
        // piece
        const col = COLUMNS[colIdx];
        if (col !== undefined) {
          const square = col + currentRow;
          position[square as Square] = fenToPieceCode(char);
        }
        colIdx = colIdx + 1;
      }
    }
...
function isValidFen(fen: string): boolean {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (!chunk || chunk.length !== 8 || chunk.search(/[^kqrnbpKQRNBP1]/) !== -1) {
      return false;
    }
  }
